@extends('master')
@section('content')
    <main>
        <div class="container">
            <h2>Giới thiệu</h2>
            <img src="{{asset('htmlcss/img/01.jpg')}}"/>
            <div class="row " id="lists">
                <div class="col-md-4 ">
                    <i class="fa fa-clock-o fa-2x"></i>
                    <p>Thông tin sinh viên tiêu biểu</p>
                </div>
                <div class="col-md-4 ">
                    <i class="fa fa-external-link fa-2x"></i>
                    <p>Xét chọn sinh viên năm tốt dễ dàng</p>
                </div>
                <div class="col-md-4 ">
                    <i class="fa fa-line-chart fa-2x"></i>
                    <p>Quảng bá rộng dãi</p>
                </div>
                <div class="col-md-4">
                    <i class="fa fa-pencil-square-o fa-2x"></i>
                    <p>Tạo đăng ký hồ sơ sinh viên 5 tốt</p>
                </div>
                <div class="col-md-4">
                    <i class="fa fa-child fa-2x"></i>
                    <p>Nâng cao chất lượng sinh viên</p>
                </div>
                <div class="col-md-4">
                    <i class="fa fa-cubes fa-2x"></i>
                    <p>Xây dựng hình ảnh sinh viên ĐHQG</p>
                </div>

            </div>
        </div>
        <section id="gioi_thieu">
            <div class="container">
                <div class="row stats">
                    <div class="col-md-3 stat">
                        <i class="fa fa-users fa-2x"></i> 7
                        <p>Trường thành viên</p>
                    </div>
                    <div class="col-md-3 stat">
                        <i class="fa fa-universal-access fa-2x"></i> 1632
                        <p>Sinh viên 5 tốt cấp cơ sở</p>
                    </div>
                    <div class="col-md-3 stat">
                        <i class="fa fa-user-circle-o fa-2x"></i> 312
                        <p>Sinh viên 5 tốt cấp ĐHQG</p>
                    </div>
                    <div class="col-md-3 stat">
                        <i class="fa fa-bullseye fa-2x"></i> 72
                        <p>Sinh viên 5 tốt cấp Trung ương</p>
                    </div>
                </div>
        </section>
    </main>
@stop
